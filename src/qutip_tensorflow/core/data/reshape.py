...
__all__ = ["reshape_tftensor", "column_stack_tftensor",
           "column_unstack_tftensor", "split_columns_tftensor"]
...
def _column_unstack_shape_check(in_shape, rows):
    ...
    if in_shape[0] % rows:
        raise ValueError(f"""number of rows, {rows} does not divide into the
                         shape {in_shape}""")
...
def reshape_tftensor(matrix, n_rows_out, n_cols_out):
    ...
    return TfTensor._fast_constructor(
        tf.reshape(matrix._tf, out_shape),
        shape=out_shape
    )
...
def split_columns_tftensor(matrix, copy=True):
    return [TfTensor(matrix._tf[:, k], copy=copy)
            for k in range(matrix.shape[1])]


qutip.data.reshape.add_specialisations([
    (TfTensor, TfTensor, reshape_tftensor)
])
...
qutip.data.split_columns.add_specialisations([
    (TfTensor, split_columns_tftensor)
])
