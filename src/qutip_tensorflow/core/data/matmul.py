...
def matmul_DenseTensor(left, right, scale=1, out=None):
    """
    ...
    if out is None:
        return DenseTensor._fast_constructor(result, shape=shape)
    else:
        out._tf = result + out._tf


qutip.data.matmul.add_specialisations(
    [(DenseTensor, DenseTensor, DenseTensor, matmul_DenseTensor)]
)
